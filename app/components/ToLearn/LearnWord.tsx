...
import { DeleteWord, LearnedWord, playAudio } from "@/app/utils/formatters";
...
export function LearnWord({ isBlurred, search }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {words
        .filter(
          (w: any) =>
            w.name.includes(search.toLowerCase().trim()) ||
            w.ruMean.includes(search.toLowerCase().trim())
        )
        .map((w: any) => (
          <div
            key={w.id}
